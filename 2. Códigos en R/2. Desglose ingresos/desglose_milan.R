...
# Evolución del desglose de ingresos del A.C. Milan

datos %>%
  
  filter(equipo == "A.C. Milan") %>%                                         # filtramos por equipo Real Madrid
  
  filter(año %in% c(2017, 2019, 2021, 2023, 2025)) %>%                       # filtramos para que solo salgan esos años
  
  
  # tratamos al año como una categoría y que aparezca en ese orden
  
  mutate(año = factor(año, levels = c(2017, 2019, 2021, 2023, 2025))) %>%    # usamos "mutate" para modificar la columna año, 
                                                                             # "factor" convierte año en categoría
                                                                             # y "levels" ordena los años
  ...
  ggplot(aes(x = año, y = ingresos, fill = categoria)) +
  ...
  labs(
    title = "Evolución del desglose de ingresos del A.C. Milan",
    x = "Año",
    y = "Ingresos (miles de €)",
    fill = "Categoría:   "
  ) +
  ...
  theme(
    ...
    axis.text.x = element_text(size = 9),                                     # tamaño números eje x
    ...
    legend.position = "bottom"                                                # dónde ponemos la leyendas
  ) +
  
  scale_y_continuous(                                                   # modifica el eje y
    limits = c(0, 200000),                                              # límite mínimo y límite máximo
    breaks = seq(0, 200000, by = 25000),                                # saltos cada 25.000
