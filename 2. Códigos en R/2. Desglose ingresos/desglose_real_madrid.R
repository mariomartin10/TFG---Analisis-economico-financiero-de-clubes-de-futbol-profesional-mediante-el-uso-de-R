...
# Evolución del desglose de ingresos del Real Madrid

datos %>%
  
  filter(equipo == "Real Madrid C.F.") %>%                                    # filtramos por equipo Real Madrid
  
  filter(año %in% c(2005, 2010, 2015, 2020, 2025)) %>%                        # filtramos para que solo salgan esos años
  
  
  # tratamos al año como una categoría y que aparezca en ese orden
  
  mutate(año = factor(año, levels = c(2005, 2010, 2015, 2020, 2025))) %>%     # usamos "mutate" para modificar la columna año, 
  ...
  labs(
    title = "Evolución del desglose de ingresos del Real Madrid C.F.",
    x = "Año",
    y = "Ingresos (miles de €)",
    fill = "Categoría:   "
  ) +
  ...
  scale_y_continuous(                                                   # modifica el eje y
    limits = c(0, 500000),                                              # límite mínimo y límite máximo
    breaks = seq(0, 500000, by = 50000),                                # saltos cada 50.000
